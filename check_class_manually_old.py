...
parser.add_argument('-s', '--synthesis', type=str, default="ss", help="Choose fo, else, ss, hc, sg, pc")
parser.add_argument('-n', '--number', type=int, default=10, help="Choose random sampple size")
parser.add_argument('-d', '--database', type=str, default="synpro", help="Choose syndev, synpro")
...
query_col = {"Publisher": "Publisher Name"}



query = {"classification": {"$regex": "filtered_out", "$options": "i"}}
...
queries = {"fo":query, "else":queryS0, "ss":queryS1, "hc": queryS2, "sg": queryS3, "pc": queryS4}

query = queries[synt]

tot_query = query_col
tot_query.update(query)
